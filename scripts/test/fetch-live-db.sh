...
# Test environment: Fetch live database from production VPS
# This script pulls the latest database backup from live and imports into myface_test
...
IMPORT_AFTER="${IMPORT_AFTER:-true}"
...
if [[ "$IMPORT_AFTER" == "true" ]]; then
    ...
    if ! psql --host="$LOCAL_DB_HOST" --username="$LOCAL_DB_USER" -lqt | cut -d \| -f 1 | grep -qw "$LOCAL_DB_NAME"; then
        log "Creating database $LOCAL_DB_NAME ..."
        createdb --host="$LOCAL_DB_HOST" --username="$LOCAL_DB_USER" "$LOCAL_DB_NAME"
    else
        log "Database $LOCAL_DB_NAME exists, dropping and recreating for clean import ..."
        psql --host="$LOCAL_DB_HOST" --username="$LOCAL_DB_USER" -c "DROP DATABASE IF EXISTS $LOCAL_DB_NAME;"
        createdb --host="$LOCAL_DB_HOST" --username="$LOCAL_DB_USER" "$LOCAL_DB_NAME"
    fi
    
    log "Importing into test database $LOCAL_DB_NAME ..."
    gunzip -c "$LOCAL_FILE" | psql --host="$LOCAL_DB_HOST" --username="$LOCAL_DB_USER" "$LOCAL_DB_NAME"
    log "Import complete - test database ready"
else
